...
@section('content')
           ...
           <div class="animated fadeIn">
              <div class="row">
                <div class="col-md-12">  <form action="{{ url('delete-section/'.Request::segment(2))}}" method="post">
                     ...
                     <input type="hidden" name="url" value="skills">
                 ...
                 <div class="col-md-12">
                    <div class="card">
                      <div class="card-header"><strong class="card-title">Skills</strong> </div>
                        <div class="card-body">
                          @if (session('success'))
                          ...
                          <div class="clearfix">
                              ...
                              <form action="{{ url('delete-skills/'.$skills->id)}}" method="post">
                                {{ csrf_field()}}
                                {{ method_field('delete')}}
                                <input type="hidden" name="cv_id" value="{{ $skills->cv_id }}"
                             <span class="float-right">  <input type="submit" name="submit" value="Delete" class="btn btn-danger float-right m-1"></span>
                           </form>

                             <span class="float-right m-1"><a href="{{ URL::to('/')}}/edit-skills/{{ $skills->id }}" class="btn btn-primary "> Edit </a></span>
                          </div>
                         <?php $i++ ?>
                          @endforeach
                </div>
              ...
              <div>
                             <button type="button" class="fa fa-plus btn btn-primary float-right" data-toggle="modal" data-target="#exampleModal" >Add(skills)</button>
      ...
      <form action="{{ URL::to('/')}}/save-skills" method="post" novalidate="novalidate">
          {{ csrf_field()}}
          {{ method_field('PUT')}}
      ...
      $(document).ready(function(){
          ...
          jQuery(".btn_close").click(function(){
            ...
            jQuery.ajax({
                type: 'POST',
                url: '{{ URL::to('/')}}/autosave-skills-ajax/' + cv_id,
                data: {
