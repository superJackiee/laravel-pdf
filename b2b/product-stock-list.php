<?php require 'includes/app_config.php';
      ...
      $objStock = new Stock();
      $stock_array = $objStock->getStockList();
?>
<!DOCTYPE html>
<html lang="en">
   <head>
               ...
               <div class="col-sm-10 wdth-rght">
                  <div class="bg-white add-mang clearfix">
                     <div class="col-sm-12 padd-left">
                        <div class="prof">Manage Stock</div>
                     ...
                     <?php foreach ($stock_array as $stock) {
                        
                        $row_id = 'user_'.$user['user_id'];
                        
                        ?>
                        <tr id='<?=$row_id?>'>
                                 <td><?=stripslashes($stock['title'])?></td>
                                 <td>
                                    <input id="input_<?=($stock['id'])?>" type="number" name="quantity[]" id="" value="<?=(int)($stock['quantity'])?>" width="100" style="width: 100px; text-align: center;" disabled />
                                    <input type="hidden" name="product_id[]" id="" value="<?=($stock['id'])?>" />
                                 </td>
                                 <td><?=stripslashes($stock['seller'])?></td>
											<td>
                                 <a href="javascript:void(0)" class="edit-btn" data-product="<?=($stock['id'])?>">Edit</a>
                                 <a class="list-ancor" href="javascript:void(0)" data-title="<?=stripslashes($stock['title'])?>" data-product="<?=$stock['id']?>" data-user="<?=$stock['user_id']?>">Save</a></td>
										</tr>
                     <?php }?>		
									</tbody>
                        </table>
               ...
               <div class="modal-dialog">
                  ...
                  <div class="modal-contentcust-modal">
                     <div class="modal-header">
                        ...
                        <h4 class="modal-title text-center"><i class="far fa-user"></i> Tenant List</h4>
                     </div>
